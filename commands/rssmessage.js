...
module.exports = function (bot, message, command) {
  ...
  chooseFeed(bot, message, command, function(rssName, msgHandler) {
    ...
    const currentMsg = '```Markdown\n' += rssList[rssName].message ? rssList[rssName].message + '```' : 'None has been set. Currently using default message below:\n\n``````\n' + config.feedSettings.defaultMessage + '```'
    ...
    .then(function(msgPrompt) {
      ...
      customCollect.on('collect', function(m) {
        ...
        else if (m.content.toLowerCase() === 'reset') message.channel.send(`Resetting message...`).then(function(resetMsg) {
          customCollect.stop();
        ...
        else if (m.content === '{empty}' && (!rssList[rssName].embedMessage || !rssList[rssName].embedMessage.enabled || !rssList[rssName].embedMessage.properties)) return message.channel.send('You cannot have an empty message if there is no embed enabled for this feed. Try again.');
        // Set new message
        else message.channel.send(`Updating message...`).then(function(editing) {
          customCollect.stop();
