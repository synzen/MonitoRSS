...
describe(
  "UserFeedConnectionEventsService Integration",
  { concurrency: true },
  () => {
    ...
    describe("handleCreatedEvents", { concurrency: true }, () => {
      it("adds connection to matching invite", async () => {
        ...
        assert.strictEqual(
          invite1?.connections?.[0]?.connectionId,
          newConnectionId,
        );
          ...
          [
            {
              ...
              creator: { discordUserId: objectId() },
            },
          ],
        );
    ...
    describe("handleDeletedEvent", { concurrency: true }, () => {
      it("removes connections from all invites", async () => {
        ...
        const feed = await createFeed(ctx, {
          invites: [
            ...
            {
              discordUserId: user2,
              connections: [connToDelete, anotherToKeep],
            },
          ],
        });
        ...
        assert.strictEqual(invite2?.connections?.length, 1);
        assert.strictEqual(
          invite2?.connections?.[0]?.connectionId,
          anotherToKeep,
        );
      });
      ...
      it("removes multiple connections at once", async () => {
        ...
        const feed = await createFeed(ctx, {
          invites: [
            { discordUserId: user1, connections: [delete1, delete2, keep1] },
          ],
        });
