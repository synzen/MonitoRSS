...
module.exports = function(bot, message) {
  ...
  else if (timezone === 'reset' && !guildRss.timezone) return message.channel.sendMessage(`Your timezone is already at default.`).catch(err => console.log(`Promise Warning: rssTimezone 5: ${err}`));
  else if (timezone == guildRss.timezone) return message.channel.sendMessage(`Your timezone is already set as \`${guildRss.timezone}\`.`).catch(err => console.log(`Promise Warning: rssTimezone 6: ${err}`));
