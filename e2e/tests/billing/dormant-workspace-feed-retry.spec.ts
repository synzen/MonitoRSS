...
// into service. Dormancy only exists when Paddle is configured, and the E2E runner
  ...
  test("explains the subscription requirement instead of offering a doomed re-enable", async ({
    ...
    await expect(
      page.getByRole("button", {
        name: `Switch workspace, current: ${workspaceName}`,
      }),
    ).toBeVisible({ timeout: 15000 });

    // The never-subscribed workspace is dormant.
    await expect(
      page.getByText("This workspace is not subscribed"),
    ).toBeVisible({ timeout: 15000 });
    ...
    await expect(page.getByText(FAILED_REQUESTS_ALERT_TEXT)).toBeVisible({
      timeout: 15000,
    });
    await expect(
      page.getByText(
        /can't be re-enabled because the workspace is not subscribed/,
      ),
    ).toBeVisible();
    await expect(
      page.getByRole("button", { name: "Attempt to re-enable" }),
    ).not.toBeVisible();
  });
});
