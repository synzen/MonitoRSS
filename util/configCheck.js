...
module.exports = function (guildId, rssIndex, logging, initializing) {
  ...
  if (rssConfig.defaultMessage == null || rssConfig.defaultMessage == "") {
    ...
    if (logging && initializing) console.log(`RSS Config Info: ${guild.id} => Feed "${rssList[rssIndex].name}" is disabled in channel ${rssList[rssIndex].channel}, skipping...`);
    return false;
  }

  if ((rssList[rssIndex].name == null || rssList[rssIndex].name == "") && rssList.length !== 0){
    if (logging) console.log(`RSS Config Warning: ${guild.id} => Feed #${parseInt(rssIndex,10) + 1} has no name defined, skipping...`);
    valid = false;
  }
  else if (rssList[rssIndex].link == null || !rssList[rssIndex].link.startsWith("http")){
    if (logging) console.log(`RSS Config Warning: ${guild.id} => ${rssList[rssIndex].name} has no valid link defined, skipping...`);
    valid = false;
  }
  else if (rssList[rssIndex].channel == null) {
    if (logging) console.log(`RSS Config Warning: ${guild.id} => ${rssList[rssIndex].name} has no channel defined, skipping...`);
    valid = false;
  }
