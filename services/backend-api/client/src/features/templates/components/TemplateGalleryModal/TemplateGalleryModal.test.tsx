...
const defaultProps = {
  ...
  connection: { id: "connection-456" } as FeedDiscordChannelConnection,
...
describe("TemplateGalleryModal", () => {
  ...
  describe("isTemplateCompatible and getMissingFields consistency", () => {
    ...
    it("field in detectedFields but not feedFields is still available", () => {
      ...
      const detectedFields = createDetectedFields({
        [TemplateRequiredField.Image]: [{ field: "imageUrl", presentInAll: true }],
        [TemplateRequiredField.Title]: [{ field: "title", presentInAll: true }],
      });
      ...
      const testCases = [
        ...
        },
        {
          ...
          detectedFields: createDetectedFields({
            [TemplateRequiredField.Image]: [{ field: "img", presentInAll: true }],
          }),
          shouldBeCompatible: true,
        },
        {
        ...
        },
        {
          feedFields: ["title"],
          detectedFields: createDetectedFields({
            [TemplateRequiredField.Description]: [{ field: "desc", presentInAll: true }],
          }),
  ...
  describe("modal behavior", () => {
    ...
    it("calls onClose when Cancel button is clicked in editor mode", async () => {
      const user = userEvent.setup();
      const onClose = vi.fn();
      render(
        <TestWrapper>
    ...
    it("calls onSecondaryAction when Cancel is clicked in picker mode", async () => {
      const user = userEvent.setup();
  ...
  describe("action buttons", () => {
    ...
    it("calls onPrimaryAction with selected template ID", async () => {
      ...
      await user.click(screen.getByText("Use this template"));
      expect(onPrimaryAction).toHaveBeenCalledWith("rich-embed", { name: "", iconUrl: undefined });
    });
  ...
  describe("accessibility", () => {
    ...
    it("has multiple focusable elements for Tab navigation within modal", async () => {
      ...
      const modalContent = screen.getByRole("dialog");
      expect(modalContent.contains(closeButton)).toBe(true);
      expect(modalContent.contains(cancelButton)).toBe(true);
      expect(modalContent.contains(primaryButton)).toBe(true);
    });
  ...
  describe("test send functionality", () => {
    ...
    it("displays error feedback alert after failed test send", () => {
      render(
        <TestWrapper>
          <TemplateGalleryModal
            {...editorDefaultProps}
    ...
    it("calls onTestSend when Send to Discord button is clicked after error", async () => {
      const user = userEvent.setup();
      const onTestSend = vi.fn();
      render(
        <TestWrapper>
