...
describe('Int::util/maintenance/pruneCollections', function () {
  ...
  it('drops unused collections', async function () {
    ...
    const feed1Collection = Article.getCollectionID(feed1.url, guildIdsByShard.get(feed1.guild), schedule1.name)
    const feed2Collection = Article.getCollectionID(feed2.url, guildIdsByShard.get(feed2.guild), schedule2.name)
    // Feeed 3 is not in guild ids, thus this collection should be deleted
    const feed3Collection = Article.getCollectionID(feed3.url, 10, schedule3.name)
    await Promise.all([
