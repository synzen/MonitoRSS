...
test.describe("Connection Settings", () => {
  test("can view connection page", async ({ page, testFeedWithConnection }) => {
    ...
    await page.screenshot({ path: "test-results/connection-page.png" });
  });
  ...
  test("can update connection channel via configure dialog", async ({
    ...
    await page
      .locator('[role="option"]')
      .filter({ hasText: "e2e-test-channel-2" })
      .click({ timeout: 15000 });
  ...
  test("can update webhook connection branding via configure dialog", async ({
    page,
    testFeed,
  }) => {
    ...
    await setSupporterStatusInDb(discordUserId);
...
test.describe("Copy Connection Settings", () => {
  ...
  test("branding settings copy to all connections", async ({
    page,
    testFeed,
  }) => {
    const channelId = getTestChannelId();
