  ...
  function selectFeed() {
    ...
    collector.on('collect', function(m) {
      ...
      if (commandList[command].action === 'Feed Removal') {
        ...
        if (invalidChosens.length > 0) return message.channel.send(`The numbers \`${invalidChosens}\` are invalid. Try again.`).then(m => msgHandler.add(m)).catch(err => console.log(`Promise Warning: chooseFeed 4: ${err}`));
        else {
          collector.stop();
