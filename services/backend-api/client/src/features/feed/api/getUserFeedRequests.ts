...
export type GetUserFeedRequestsOutput = InferType<
typeof GetUserFeedRequestsOutputSchema
>;

export const getUserFeedRequests = async (
  { feedId, data }: GetUserFeedRequestsInput,
): Promise<GetUserFeedRequestsOutput> => {
  const query = qs.stringify(data);

  const res = await fetchRest(
    `/api/v1/user-feeds/${feedId}/requests?${query}`,
    {
      requestOptions: {
        method: 'GET',
      },
