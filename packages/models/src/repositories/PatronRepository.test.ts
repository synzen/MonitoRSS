...
import PatronRepository from './PatronRepository';

describe('PatronRepository', () => {
  let repo: PatronRepository;
  let db: Db;
  const collectionName = PatronRepository.COLLECTION_NAME;
  ...
  beforeAll(async () => {
    ...
    repo = PatronRepository.getRepository(db);
  });
  beforeEach(async  () => {
