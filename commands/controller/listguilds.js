...
exports.normal = async (bot, message) => {
    try {
        ...
        let txt = ''
        guilds.forEach(item => txt += `[${item.members}] (G: ${item.id}, ${item.name}) ${item.owner ? `(O: ${item.owner.id}, ${item.owner.name})` : `(O: None)`} \n`)
        await m.delete()
...
exports.sharded = async (bot, message) => {
    try {
        ...
        let txt = ''
        allGuilds.forEach(item => txt += `[${item.members}] (G: ${item.id}, ${item.name}) ${item.owner ? `(O: ${item.owner.id}, ${item.owner.name})` : `(O: None)`} \n`)
        await m.delete()
        await message.channel.send({
            files: [
                {
