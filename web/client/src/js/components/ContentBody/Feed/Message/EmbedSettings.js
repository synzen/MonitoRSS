...
import { Input, Dropdown, Popup, Button, Divider, Checkbox, Icon } from 'semantic-ui-react'
...
import embedPropertiesNames from '../../../../constants/embed.js'
      ...
      <div>
        <TwoColumn>
          <div>
            ...
            <Input onChange={e => onUpdate(embedPropertiesNames.title, e.target.value)} value={valuesToUse[embedPropertiesNames.title] || ''} />
          </div>
          <div>
            <SectionSubtitle>Title URL</SectionSubtitle>
            <Input onChange={e => onUpdate(embedPropertiesNames.url, e.target.value)} value={valuesToUse[embedPropertiesNames.url] || ''} />
          </div>
        </TwoColumn>
        <TwoColumn>
          <div>
            <SectionSubtitle>Footer Text</SectionSubtitle>
            <Input onChange={e => onUpdate(embedPropertiesNames.footerText, e.target.value)} value={valuesToUse[embedPropertiesNames.footerText] || ''} />
          </div>
          <div>
            <SectionSubtitle>Footer Icon URL</SectionSubtitle>
            <Popup
              inverted
              hideOnScroll
              content='This will only appear if the Footer Text is specified'
              on='focus'
              position='bottom left'
              trigger={<Input onChange={e => onUpdate(embedPropertiesNames.footerIconUrl, e.target.value)} value={valuesToUse[embedPropertiesNames.footerIconUrl] || ''} />}
            />
          </div>
        </TwoColumn>
        ...
        <TwoColumn>
          <div>
            <SectionSubtitle>Author Name</SectionSubtitle>
            <Input onChange={e => onUpdate(embedPropertiesNames.authorName, e.target.value)} value={valuesToUse[embedPropertiesNames.authorName] || ''} />
          </div>
          <div>
            <SectionSubtitle>Author Icon URL</SectionSubtitle>
            <Popup
              inverted
              hideOnScroll
              content='This will only appear if the Author Name is specified'
              on='focus'
              position='bottom left'
              trigger={<Input onChange={e => onUpdate(embedPropertiesNames.authorIconUrl, e.target.value)} value={valuesToUse[embedPropertiesNames.authorIconUrl] || ''} />}
            />
          </div>
        </TwoColumn>
        ...
        <TwoColumn>
          <div>
            <SectionSubtitle>Thumbnail URL</SectionSubtitle>
            <Input onChange={e => onUpdate(embedPropertiesNames.thumbnailUrl, e.target.value)} value={valuesToUse[embedPropertiesNames.thumbnailUrl] || ''} />
          </div>
          <div>
            <SectionSubtitle>Image URL</SectionSubtitle>
            <Input onChange={e => onUpdate(embedPropertiesNames.imageUrl, e.target.value)} value={valuesToUse[embedPropertiesNames.imageUrl] || ''} />
          </div>
        </TwoColumn>
        ...
        <TwoColumn>
          <div style={{ display: 'flex' }}>
            ...
            <InputWithButtonGroup>
              <Input value={valuesToUse[embedPropertiesNames.color] || ''} />
              <Button.Group>
                <Popup
                ...
                >
                  <DiscordColorPicker color={numberToColour(valuesToUse[embedPropertiesNames.color] || 0)} onChange={(color, event) => {
                    onUpdate(embedPropertiesNames.color, ((color.rgb.r << 16) + (color.rgb.g << 8) + color.rgb.b).toString()) // Convert to integer
                  }} />
                </Popup>
                <Button icon='x' disabled={!valuesToUse[embedPropertiesNames.color]} onClick={e => onUpdate(embedPropertiesNames.color, '')} />
              </Button.Group>
            </InputWithButtonGroup>
          </div>
          <div>
            <SectionSubtitle>Timestamp</SectionSubtitle>
            <Dropdown selection options={[{ text: 'None', value: 'none' }, { text: 'article', value: 'article' }, { text: 'now', value: 'now' }]} onChange={(e, data) => onUpdate(embedPropertiesNames.timestamp, data.value)} value={valuesToUse[embedPropertiesNames.timestamp] || 'none'} />
          </div>
        </TwoColumn>

        <SectionSubtitle style={{ marginTop: '2em' }}>Description</SectionSubtitle>
        <TextArea onChange={e => onUpdate(embedPropertiesNames.description, e.target.value)} value={valuesToUse[embedPropertiesNames.description] || ''} lineCount={valuesToUse[embedPropertiesNames.description] ? valuesToUse[embedPropertiesNames.description].split('\n').length : 0} />
        <SectionSubtitle style={{ marginTop: '2em' }}>Fields</SectionSubtitle>
        <EmbedFieldsWrapper>
          <Scrollbars>
            <EmbedFieldsWrapperInner>
              <PoseGroup>
                {fieldElements}
                {/* <AddEmbedFieldBox onClick={this.addEmbedField}>
