    ...
    if (oldMemberHas !== newMemberHas) {
      if (!newMemberHas) {
        RedisGuildMember.utils.forgetManager(newMember)
          .catch(err => log.general.error(`Redis failed to forgetManager after guildMemberUpdate event`, newMember.guild, newMember, err))
      } else {
        RedisGuildMember.utils.recognizeManager(newMember)
          .catch(err => log.general.error(`Redis failed to recognizeManager after guildMemberUpdate event`, newMember.guild, newMember, err))
      }
    }
  }
}
