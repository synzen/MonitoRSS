...
function dump (m, collections, complete = 0) {
  ...
  if (!collection) {
    log.owner.info('Database backup complete', m.author)
    return m.edit(`Dumped ${complete}/4 total collections.${complete > 0 ? `. See \`${BACKUP_PATH}\` for the dump folder.` : ''}`).catch(err => log.owner.warning('dbbackup', m.author, err))
  }
  log.owner.info(`Attempting to dump collection ${mongoose.connection.name}.${collection}`)
