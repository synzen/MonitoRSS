...
module.exports = function (bot, message, command) {
  ...
  const menu = new Discord.RichEmbed()
    ...
    .addField('2) Toggle Image Link Previews for a feed', `Default is ${config.feedSettings.imgPreviews === false ? 'disabled' : 'enabled'}. Toggle automatic Discord image link embedded previews for image links found inside placeholders such as {description}.`)
    .addField('3) Toggle Image Links Existence for a feed', `Default is ${config.feedSettings.imgLinksExistence === false ? 'disabled' : 'enabled'}. Remove image links found inside placeholders such as {description}. If disabled, all image \`src\` links in such placeholders will be removed.`)
