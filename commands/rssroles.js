...
async function printSubscriptions (message, rssList) {
  ...
  for (const rssName in rssList) {
    ...
    if (source.roleSubscriptions) {
      for (const globalSubber in source.roleSubscriptions) {
        ...
        const globalSubbedRole = guild.roles.get(source.roleSubscriptions[globalSubber].roleID).name
        subList[source.title].globalSubs.push(globalSubbedRole)
      }
    } else if (source.userSubscriptions) {
      for (const globalSubber in source.userSubscriptions) {
        if (!subList[source.title]) subList[source.title] = {}
        if (!subList[source.title].globalSubs) subList[source.title].globalSubs = []

        const globalSubbedUser = guild.members.get(source.userSubscriptions[globalSubber].userID).user
  ...
  if (Object.keys(subList).length === 0) await message.channel.send(`There are no subscriptions for the feed <${rssList[rssName].link}>.`)
  else {
...
async function deleteSubscription (message, guildRss, role, user) {
  ...
  for (const index in rssList) {
    ...
    if (roleID && source.roleSubscriptions) {
      ...
      for (const globalSubber in source.userSubscriptions) {
        if (source.userSubscriptions[globalSubber].userID === userID) {
  ...
  if (!found) await message.channel.send(`This ${role ? 'role' : 'user'} has no subscriptions to remove.`)
    ...
    await message.channel.send(`All subscriptions successfully deleted for ${role ? `role ${role.name}\`` : `user \`${user.username}#${user.discriminator}\``}. After completely setting up, it is recommended that you use ${config.bot.prefix}rssbackup to have a personal backup of your settings.`)
  }
}
...
async function addGlobalSub (message, guildRss, rssName, role, user) {
  ...
  if (role) {
    ...
    for (const globalSubber in source.userSubscriptions) {
      if (source.userSubscriptions[globalSubber].userID === user.id) return message.channel.send(`Unable to add global subscription. User \`${user.username}#${user.discriminator}\` is already subscribed to this feed.`)
    }
  ...
  await dbOps.guildRss.update(guildRss)
  await message.channel.send(`Global subscription successfully added for \`${role ? role.name : `${user.username}#${user.discriminator}`}\` to feed <${source.link}>. After completely setting up, it is recommended that you use ${config.bot.prefix}rssbackup to have a personal backup of your settings.`)
}
...
async function removeGlobalSub (message, guildRss, rssName, role, user) {
  ...
  if (role) {
    ...
    for (const globalSubber in source.userSubscriptions) {
      if (source.userSubscriptions[globalSubber].userID === user.id) {
        source.userSubscriptions.splice(globalSubber, 1)
        found = true
      }
    }
    if (source.userSubscriptions.length === 0) delete source.userSubscriptions
    if (!found) return message.channel.send(`The user \`${user.username}#${user.discriminator}\` does not have a global subscription to this feed.`)
  }
...
async function globalSubMenuFn (m, data) {
  ...
  if (input === '1') {
    ...
    if (!source.roleSubscriptions) return m.channel.send(`There are no ${role ? 'role' : 'user'} global subscriptions to remove from the feed <${source.link}>.`)
...
async function getUserOrRoleFn (m, data) {
  ...
  return { ...data, role, user: member.user }
}
