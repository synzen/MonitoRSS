...
function testUrl(feedId: string, connectionId: string) {
  return `/api/v1/user-feeds/${feedId}/connections/discord-channels/${connectionId}/preview`;
}
...
describe(
  "POST /api/v1/user-feeds/:feedId/connections/discord-channels/:connectionId/preview",
  { concurrency: true },
  () => {
    ...
    it("returns 201 for owner", async () => {
      ...
      const body = (await response.json()) as {
        result: { status: string; customPlaceholderPreviews: string[][] };
      };
    ...
    it("returns 201 when article is omitted", async () => {
      const discordUserId = generateSnowflake();
      const user = await ctx.asUser(discordUserId);
      const { feedId, connectionId } = await createTestFeedWithConnection(ctx, {
        discordUserId,
      ...
      assert.strictEqual(body.result.status, "SUCCESS");
      assert.ok(Array.isArray(body.result.customPlaceholderPreviews));
    });

    it("returns 201 when article is provided", async () => {
      const discordUserId = generateSnowflake();
      const user = await ctx.asUser(discordUserId);
      const { feedId, connectionId } = await createTestFeedWithConnection(ctx, {
        discordUserId,
