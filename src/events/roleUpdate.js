...
module.exports = (oldRole, newRole) => {
  if (RedisRole.clientExists) {
      ...
      if (!newRoleHas) {
        newRoleMembers.forEach(member => {
          RedisGuildMember.utils.forgetManager(member)
            .catch(err => log.general.error(`Redis failed to members.forgetManager after roleUpdate event`, member.guild, member, err))
        })
        RedisRole.utils.forgetManager(newRole)
          .catch(err => log.general.error(`Redis failed to roles.forgetManager after roleUpdate event`, newRole.guild, newRole, err))
      } else {
        newRoleMembers.forEach(member => {
          RedisGuildMember.utils.recognizeManager(member)
            .catch(err => log.general.error(`Redis failed to members.recognizeManager after roleUpdate event`, member.guild, member, err))
        })
    ...
    RedisRole.utils.update(oldRole, newRole)
      .catch(err => log.general.error(`Redis failed to update role after roleUpdate event`, err))
  }
}
