    ...
    createContext(options: UsersContextOptions = {}): UsersContext {
      ...
      const userRepository: MockUserRepository = {
        findByDiscordId: mock.fn(
          options.userRepository?.findByDiscordId ?? (() => Promise.resolve(null))
        ),
        findIdByDiscordId: mock.fn(
          options.userRepository?.findIdByDiscordId ?? (() => Promise.resolve(null))
        ),
        create: mock.fn(
          options.userRepository?.create ?? (() => Promise.resolve(DEFAULT_USER))
        ),
        updateEmailByDiscordId: mock.fn(
          options.userRepository?.updateEmailByDiscordId ?? (() => Promise.resolve(DEFAULT_USER))
        ),
        updatePreferencesByDiscordId: mock.fn(
          options.userRepository?.updatePreferencesByDiscordId ?? (() => Promise.resolve(DEFAULT_USER))
        ),
        findEmailsByDiscordIdsWithAlertPreference: mock.fn(
          options.userRepository?.findEmailsByDiscordIdsWithAlertPreference ?? (() => Promise.resolve([]))
        ),
        setExternalCredential: mock.fn(
          options.userRepository?.setExternalCredential ?? (() => Promise.resolve())
        ),
        getExternalCredentials: mock.fn(
          options.userRepository?.getExternalCredentials ?? (() => Promise.resolve(null))
        ),
        removeExternalCredentials: mock.fn(
          options.userRepository?.removeExternalCredentials ?? (() => Promise.resolve())
        ),
        revokeExternalCredential: mock.fn(
          options.userRepository?.revokeExternalCredential ?? (() => Promise.resolve())
        ),
        aggregateUsersWithActiveRedditCredentials: mock.fn(
          options.userRepository?.aggregateUsersWithActiveRedditCredentials ?? emptyGenerator
        ),
        aggregateUsersWithExpiredOrRevokedRedditCredentials: mock.fn(
          options.userRepository?.aggregateUsersWithExpiredOrRevokedRedditCredentials ?? emptyGenerator
        ),
      };

      const userFeedRepository: MockUserFeedRepository = {
        bulkUpdateLookupKeys: mock.fn(
          options.userFeedRepository?.bulkUpdateLookupKeys ?? (() => Promise.resolve())
        ),
      };

      const supporterRepository: MockSupporterRepository = {
        findById: mock.fn(
          options.supporterRepository?.findById ?? (() => Promise.resolve(null))
        ),
      };
      ...
      const supportersService: MockSupportersService = {
        ...
        getSupporterSubscription: mock.fn(
          options.supportersService?.getSupporterSubscription ?? (() => Promise.resolve({ customer: null, subscription: null }))
        ),
      };
      ...
      const paddleService: MockPaddleService = {
        ...
        getCustomerCreditBalanace: mock.fn(
          options.paddleService?.getCustomerCreditBalanace ?? (() => Promise.resolve({ data: [] }))
        ),
      };
