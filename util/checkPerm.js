...
module.exports = function (content, bot, channel) {
  ...
  if (!guildBot.permissionsIn(channel).hasPermission("SEND_MESSAGES")) {
    channel.sendMessage("A guild permission error has been found. I am currently trying to fix thus, but in the meanwhile try recreating the bot's role if it exists, or specifically adding the bot's name to permissions.").then(m => console.log("error: message actually sent.")).catch(err => console.log("Successful block"));
