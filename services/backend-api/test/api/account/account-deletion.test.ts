...
describe("Account deletion API", () => {
  ...
  it("refuses to send a deletion code to the sole owner of a workspace", async () => {
  ...
  it("blocks deletion when the user became a sole owner after the code was sent", async () => {
    const user = await seedUser();
  ...
  it("sends the dedicated account-deletion email, not the verify-email template", async () => {
    const user = await seedUser();
    const authed = await ctx.asUser(user.discordUserId);
    await requestCode(authed);

    assert.strictEqual(sent.length, 1);
    assert.strictEqual(sent[0]!.to, user.verifiedEmail);
    assert.strictEqual(
      sent[0]!.subject,
      "Confirm your MonitoRSS account deletion",
    );
    assert.ok(sent[0]!.html.includes("Confirm account deletion"));
    assert.ok(
      !sent[0]!.html.includes("Verify your email"),
      "Deletion email must not present itself as email verification",
    );
  });

  it("rejects a code issued for email verification at the deletion endpoint", async () => {
    const user = await seedUser();
    const authed = await ctx.asUser(user.discordUserId);

    // Request a code through the ordinary verify-email route for the same
    // address the deletion flow would target.
    const sendRes = await authed.fetch("/api/v1/users/@me/email-verification", {
      method: "POST",
      body: JSON.stringify({ email: user.verifiedEmail }),
    });
    ...
    const code = sent
      .map((m) => m.html.match(/class="email-code"[^>]*>\s*(\d{6})\s*</)?.[1])
      .find((c): c is string => !!c);
    assert.ok(code, "Expected a 6-digit code in the captured email");
    ...
    assert.ok(stillThere, "A verify-email code must not delete the account");
  });

  it("rejects a deletion code at the email-verification confirm endpoint", async () => {
    const user = await seedUser();
    const authed = await ctx.asUser(user.discordUserId);
    const code = await requestCode(authed);
    ...
    const res = await authed.fetch(
      ...
      {
        method: "POST",
        body: JSON.stringify({ email: user.verifiedEmail, code }),
      },
    );
